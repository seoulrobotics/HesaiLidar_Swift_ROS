...
class PandarDriver {
 public:
  PandarDriver(ros::NodeHandle node, ros::NodeHandle private_nh,
               std::string nodetype, pandar_pointcloud_at128::Convert *convert);
  ~PandarDriver() {}
 ...
 private:
  ...
  void callback(pandar_pointcloud_at128::CloudNodeConfig &config, uint32_t level);  //no use
  ...
  pandar_pointcloud_at128::Convert *convert;
  bool m_bPaserPacp;
};
