...
#include <pandar_pointcloud_at128/platUtil.h>
...
Convert::Convert(ros::NodeHandle node, ros::NodeHandle private_nh,
    ...
    : data_(new pandar_rawdata_at128::RawData()),
  ...
  if (LIDAR_NODE_TYPE == node_type) {
    ...
    dynamic_reconfigure::Server<
        pandar_pointcloud_at128::CloudNodeConfig>::CallbackType f;
...
void Convert::callback(pandar_pointcloud_at128::CloudNodeConfig &config,
                       uint32_t level) {
...
void Convert::processScan(const pandar_msgs::PandarScan::ConstPtr &scanMsg) {
  ...
  pandar_rawdata_at128::PPointCloud::Ptr outMsg(new pandar_rawdata_at128::PPointCloud());
